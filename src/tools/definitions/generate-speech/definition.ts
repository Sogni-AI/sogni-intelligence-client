/**
 * Tool definition for generate_speech — Qwen3-TTS text-to-speech.
 *
 * The failure mode this description exists to prevent: an agent used to image
 * prompts writing "a calm woman reading the news" into `prompt` and having those
 * seven words read aloud. The script is literal; everything about the voice goes
 * somewhere else.
 */
...
export const definition: ToolDefinition = {
  ...
  function: {
    ...
    description:
      'Turn written words into spoken audio. Use when the user wants something read aloud, a narration, a voiceover, a line of dialogue, an audiobook passage, or a voice cloned from a recording they uploaded. This tool speaks text; it does not compose music — use generate_music for songs and instrumentals.',
    parameters: {
      type: 'object',
      properties: {
        prompt: {
...
Limit 4096 characters, roughly five minutes of speech. A longer script is refused rather than cut off mid-sentence, so split it across several calls at a natural break.`,
        ...
        voice: {
          ...
          description:
            'Studio voice to speak in. Only used when model="voice". Female: serena (English), vivian (Chinese), ono_anna (Japanese), sohee (Korean). Male: ryan, aiden, eric, dylan (English), uncle_fu (older, Chinese). Every voice speaks all supported languages — the note above describes the accent it carries, not a limit on what it can read. Default: serena.',
        },
        voiceDescription: {
          type: 'string',
          description:
            'How the line should be delivered, or who should deliver it. With model="voice" this restyles the chosen voice without changing who it is: "whispering, close to the microphone", "furious", "reading a bedtime story", "like a sports commentator". With model="design" this is required and describes a speaker to invent: age, gender, accent, timbre, pace, mood, and recording space — "a warm, unhurried narrator in her forties with a faint Scottish lilt, close-miked in a quiet room". Describe one coherent person; contradictory directions produce a voice that shifts mid-sentence. Not accepted with model="clone", where the recording defines the voice. Limit 512 characters.',
          maxLength: 512,
        },
        voiceSourceIndex: {
          type: 'number',
        ...
        voiceTranscript: {
          type: 'string',
          description:
            'The exact words spoken in the uploaded clip, when model="clone". Supplying it lets the model condition on the recording itself rather than on a speaker fingerprint alone, which is markedly closer to the source — set it whenever the user tells you what the clip says or the transcript is otherwise known. Omitting it still produces a recognisable clone, just a looser one. Limit 1024 characters.',
          maxLength: 1024,
        },
        language: {
          type: 'string',
        ...
        numberOfVariations: {
          type: 'number',
          description:
            'Number of takes (1-16). Each take is a separate read of the same script with the same voice, differing only in delivery. Use more than 1 when the user asks for options or alternate reads. Default: 1.',
          minimum: 1,
          maximum: 16,
        },
      },
