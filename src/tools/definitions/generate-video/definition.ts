...
BATCH VARIATIONS: When numberOfVariations > 1, use Dynamic Prompt syntax. This is one Sogni project with multiple jobs, so prefer it when all outputs share the same references, model, duration, dimensions, and generation parameters and only prompt text varies. Lock in any camera/subject/style the user specified, vary the rest. Example: "slow dolly in on a city street {at dawn with golden light|during a rainstorm|at night with neon reflections}".`,
        ...
        width: {
          type: "number",
          description:
            'Video width in pixels. LTX 2.3: 640-3840. WAN 2.2: each side 480-1536 and at most 1,048,576 pixels per frame (1024x1024), so 1280x720, 720x1280, 1024x1024, and 1440x720 fit but 1920x1080 and 1536x864 do not. WAN 2.2 refuses a larger size instead of shrinking it: when the user asks WAN 2.2 for more pixels, say it cannot render that size and offer the largest WAN 2.2 size of that shape or another model. Default resolution depends on model and quality tier: LTX Fast about 720p and High/Pro about 1080p; WAN Fast uses 480p short side and High/Pro uses 720p short side, smaller for very wide or tall shapes so the frame stays within 1,048,576 pixels. Set width only when the user specifies an exact width or orientation-qualified exact pixels. A bare named resolution like "720p resolution" is a short-side target, not an instruction to make landscape 1280x720. If the user gives only one exact dimension, set only that dimension and preserve/infer the sensible aspect ratio. User-requested exact dimensions override the default media quality. Mappings when orientation is explicit: 480p landscape=854x480, 480p portrait=480x854, 720p landscape=1280x720, 720p portrait=720x1280, 1080p landscape=1920x1080, 1080p portrait=1080x1920, 4K landscape=3840x2160 (1080p and 4K exceed WAN 2.2). Non-step values are accepted when in bounds; LTX snaps to the nearest 64px step and WAN snaps to the nearest 16px step internally, so do not ask the user to adjust by a few pixels.',
        },
        height: {
          type: "number",
          description:
            'Video height in pixels. LTX 2.3: 640-3840. WAN 2.2: each side 480-1536 and width x height at most 1,048,576 pixels; WAN 2.2 refuses a larger size instead of shrinking it. Set height only when the user specifies an exact height or orientation-qualified exact pixels. A bare named resolution like "720p resolution" is a short-side target; do not convert it to landscape dimensions unless the user says landscape/horizontal/widescreen. If the user gives only one exact dimension, set only that dimension and preserve/infer the sensible aspect ratio. User-requested exact dimensions override Default Media Quality, including Pro. Non-step values are accepted when in bounds; LTX snaps to the nearest 64px step and WAN snaps to the nearest 16px step internally, so do not ask the user to adjust by a few pixels.',
        },
        targetResolution: {
          type: "number",
          description:
